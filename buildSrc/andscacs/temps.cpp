...
int buffer_de_temps;//guardar una mica de temps de m�s, per si de cas
int movimentsperfer;  //n�mero de moviments que es considera que queden fins el final de la partida.
...
void calcular_temps_moviment() {
	...
	if (temps_tipus & TipusNumMoviments)
		movimentsperferx = temps_NumMoviments + 2;  //si �s un tipus de partida amb control, afegir dues jugades a les jugades que queden pel control per tenir una mica de buffer
	...
	if (temps_tipus & TipusTemps) {
		int tempsx = temps_temps[color_que_juga_pc] - (4 * resolucio_rellotge); //per deixar un m�nim

		if (ssbase[0].moviment_a_ponderar != darrer_moviment)
			movimentsperferx -= 1;  //pensar una mica m�s si no han fet la jugada que esperava

		temps_per_moure += tempsx / movimentsperferx;
		temps_maxim = tempsx / _min(14, movimentsperferx);
		//si tenim m�s temps que el rival, podem pensar una mica m�s
