...
ClauHash ClauInici; //per no comen�ar a 0
ClauHash ClauTorn;
ClauHash ClauPeca[16][64]; //fins el n�mero del rei negre, que �s el m�s gran
ClauHash ClauEnroc[16]; //combinacions de 4 bits
ClauHash ClauEP[8];
ClauHash ClauMovimentExclos; //s'usa per diferenciar els hash guardats dels moviments exclosos com a conseq��ncia del Singular extension search
...
{
	ComptadorsHash xx;
	struct thash unahash; //nom�s per tenir sizeof
...
bool usar_entrada(struct thash *auxhashb, ClauGuardaHash key32, Moviment *m){
	//si no ha trobat la clau de la posici�
...
void guardar_hash(int score, int n, int nivell, ClauHash hashposicio, uint8_t tt_flag, Moviment moviment, Moviment segonmoviment, Valor avaluacio) {
	...
	int i;
	int a1 = (int)h1->edat << 3;
	int a2 = (int)h2->edat << 3;
	int a3 = (int)h3->edat << 3;
	int a4 = (int)h4->edat << 3;
