...
#define PenalitzacioPecaAtacaCasellaDavantDebil FerPun(2, 3) //(2, 5) //no: bo a r�pides (4, 5)
#define BonificacioDebilnoAtacadaNiOcupada FerPun(3, 3)
#define PenalitzacioPecaAtacaDebil FerPun(3, 8) //(3, 8) //no: (3, 10), no est� clar (3, 6)
...
//si en final el rei no est� prop de la columna m�s propera que t� peons de qualsevol color
//** potser no �s bo per final on hi ha dames
...
int vp_peons[64] = { // De cara a el negre, que est� avall
	0, 0, 0, 0, 0, 0, 0, 0,
	-7, -2, 1, 6, 6, 1, -2, -7,
	-7, -2, 3, 14, 14, 3, -2, -7,
	-7, -2, 7, 21, 21, 7, -2, -7,
...
void precalcul_estructura_peons() {
	...
	for (i = 0; i < 256; i++) {
		...
		for (j = 0; j < 8; j++) { //j=columna on estaria el rei
			PenalitzacioDistanciaReiColumnaPeons[j][i] = 0; //per defecte no hi ha penalitzaci�
			//si no hi ha peons, no penalitzaci�
			if (i == 0)
				continue;
			//si hi ha peons a la columna del rei, no penalitzaci�
			if (TestBit(i, j))
				continue;
			//buscar columna esquerra m�s propera al rei que t� peons
			for (k = j - 1; k >= 0; k--)
				if (TestBit(i, k))
					break;
			//buscar columna dreta m�s propera al rei que t� peons
			for (l = j + 1; l <= 7; l++)
				if (TestBit(i, l))
					break;
			//si hi ha peons a les columnes anteriors i posteriors del rei, no hi ha penalitzaci�
			if (k >= 0 && l <= 7 && j >= k && j <= l)
				continue;
			if (k >= 0)
				m = j - k;
...
#define MulPeonsDavant 10 //12  //no 8, 13 //14 un pel millor a r�pides o molt poquet millor
...
void carregar_fitxer_avaluacions() {
	...
	//per SEE, id�ntic al normal per� alfil=caball pq no descarti canvis entre ells
	...
	PenalitzacioAlfilBloquejatPeoCentral2 = FerPun(-1, 3); //tunejat (-1, 3) //no: (0, 3) i (1,3), els dos millor a r�pides
	...
	Lever[0][5] = FerPun(24, 21); //(24, 21) //no: bo a r�pides: (19, 18)
	...
	BonificacioPassatAmbSuport = FerPun(1, 16); //(1, 16)  //1,10 �s bastant pitjor que 1,20
...
int avaluacio_caballs(tss * RESTRICT ss, struct InfoEval *ie) {
	...
	for (a = Tcaballs(jo); a != 0; a &= a - 1, i++) {
		...
		//mobilitat. Atacs de la pe�a: treure atacs peons.
		...
		if (bb & Bit(c)) { //si no pot ser atacat per un peo rival
			...
			if (PunI(j) >= 0) //si pot ser expulsat per pe�, el valor posici� �s la meitat (si no negatiu)
		...
		if (ie->claven[opo] & Bit(c)) {
			if (j > 0)
				j--; //si caball est� clavat, menys mobilitat
			if (!(estat_actual->atacspeons[jo] & Bit(c)) && !(CasellesDavant[jo][c] & (Tpeons(jo) | Tpeons(opo)))) //si no t� peons davant
				pun -= FerPun(10, 0); //clavat en una columna oberta sense defensar per pe� propi
		}
		...
		//caball val menys si no pot anar a flanc contrari i est� al flanc propi
		...
		atacscaball = atacscaball & (~Tpeces(jo)); //treure caselles on hi ha peces pr�pies
	...
	for (; a != 0; a &= a - 1, i++) {
		...
		if (color_casella(c) == blanques) {
			...
			if (jo == blanques) {
				...
				//el bloqueig del peo a c4 �s menys important
				...
				//el bloqueig del peo a f5 �s menys important
				if (Tpeons(negres) & F5) {
			...
			if (jo == blanques) {
				...
				//el bloqueig del peo a f4 �s menys important
				...
				//el bloqueig del peo a c5 �s menys important
				if (Tpeons(negres) & C5) {
		...
		if (jo == blanques) {
			...
			//�lfil clavant caball contra dama, al mateix flanc on �s el rei, bonificaci�
		...
		//atacs de la pe�a. treure atacs peons.
		atacsrei2 = atacs & (~ie->atacspeons_per_mobilitat[jo]); //** ho feia diferent. no cal
		//n�mero de caselles atacades restants
		j = popcount(atacsrei2);
		bool clavat = false;
		//si pe�a est� clavada, menys mobilitat
		if (j > 0 && (ie->claven[opo] & Bit(c))) {
			j--;
			clavat = true;
		}
		if (DarreraPeonsCentralsMobils) //alfil no �s tant dolent si darrera peons centrals mobils
			j++;
		if (j < 0)
			j = 0;
		if (j > 13)
	...
	for (; a != 0; a &= a - 1, i++) {
		...
		if (fila == fila7) {
			if (ROW(ie->rei[opo]) == fila8) {
				punt += BonusTorreTallaRei7a; //nom�s afecta al final
			}
		}
		if (jo == blanques)
		...
		//mobilitat
		//atacs de la pe�a. treure atacs peces menors valor
		...
		//penalitzaci� dama al flanc contrari del rei contrari i no pot tornar
		...
		bool atac = false;
		if (!peopropidavant) {  //si la torre no t� peons propis davant  //** en comptes de CasellesDavant, aqu� i a altres llocs, es pot usar un CasellesDavant de nom�s una fila, i fer shift per la columna
			punt2 = 0;
			...
			if (abs(columna - COL(ie->rei[opo])) < 2 && fila_relativa(jo, ROW(lsb(peopropidavant))) > 3 && (estat_actual->atacspeons[opo] & peopropidavant))
				atac = true; //est� obrint columna sobre el rei
		}
...
int avaluacio_torres(tss * RESTRICT ss, struct InfoEval *ie) {
	...
	//penalitzaci� per torres bloquejades pel rei
	if (jo == blanques) {
...
int avaluacio_rei(tss * RESTRICT ss, struct InfoEval *ie) {
	...
	//si el rei no pot moure, penalitzaci�
	...
	if ((ie->num_peces_ataquen[jo] > 0 || AtacsQuadre > 3 || ReiAvancat || PeonsDavant > 1))
	{
		...
		//si est� trencant amb un pe� prop del rei, incrementar valor atac
		int atacapeons = 0;
		if (quadrerei) {
			...
			//si avan�ant pot trencar. Ho vaig desfer i a 35 segons era m�s bo, per� a 66 semblava igual o un pel pitjor
			if (ShiftPerPeons(jo, estat_actual->atacspeons[opo], 8) & Tpeons(jo) & quadrerei) {
				i += 3; //3 //no: 5
				atacapeons = 1;
			}
			//caselles d�bils prop rei (3a fila), i no �lfil
			if (!ReiAvancat) {
		...
		x = 0;
		if (atacsrei) {
			atacsrei &= estat_actual->atacspeons[opo] | estat_actual->atacscaballs[opo] | estat_actual->atacsalfils[opo] | estat_actual->atacsdames[opo] | ie->bitboard_casellesrei[opo]; //es treuen les torres perqu� estan suportant la seva pr�pia casella d'escac
			if (atacsrei) {
				x = EscacContacteTorre * popcount(atacsrei) * (estat_actual->mou == opo ? 2 : 1); //si pot fer escac i li toca, m�s penalitzaci�
			}
		}
		...
		if (atacsrei) {
			...
			if (atacsrei) {
				x += EscacContacteDama * popcount(atacsrei) * (estat_actual->mou == opo ? 2 : 1); //si pot fer escac i li toca, m�s penalitzaci�
			}
		}
		...
		if (segures2) {
			...
			if (b1) {
				...
				x += w;
				//si no havia posat que les torres atacaven, per� tenen un escac, s� que ataquen
		...
		//si hi ha moltes peces defensant, menys penalitzaci�
		...
		//penalitzaci� rei 2a fila i no t� peo al costat llarg
		if ((thp->flags & Bit(jo))
		...
		//contrari t� m�s peons davant del rei. idea hakkapelitta
		...
		//redu�r pot�ncia atac si: pocs atacants, (cap menor atacant i alguna pr�pia defensant i no escacs contacte) o pot menjar dama
		...
		if (PunI(i) > 0) {
			...
			if (x > 0)
				i += x; //si el rei est� malament, treure-li el bonus
			//redu�r pot�ncia atac si dama no ataca
	...
	//si contrari m�nim dama + torre, sumar algo del valor al final
...
int avaluacio_peons_hash(tss * RESTRICT ss, struct InfoEval *ie, Bitboard *bloquejats) {
	...
	for (a = Tpeons(jo); a != 0; a &= a - 1) {
		...
		if ((!(Tpeons(opo) & AvancPeonsPassats[jo][i8])) && !(Tpeons(jo) & CasellesDavant[jo][i8])/*doblat*/) { //si �s passat //** potser marcar-ho al fer moviment, per anar m�s r�pid
			...
			if (!aillat && !connectat && !(Tpeons(jo) & (AtacsdePeons[opo][davantpeo])))
				passatsdebils |= Bit(i8); //passat d�bil
		}
		else {
			//candidats a passat
			if (!peonsdavantoponent) {
				...
				if (Tpeons(jo) & (veins & Files[r]))
					goto sicandidatpassat; //�s l'altre pe� el passer candidate, al sacrificar aquest
		...
		nocandidatpassat:
			if ((Tpeons(opo) & Bit(davantpeo))) {
				//disminuir puntuaci� base peons bloquejats
				if (PunI(j) > 0)
					i -= FerPun(PunI(j) >> 2, 0);
				if (jo == blanques)
			...
			//si peo d�bil
			if (!aillat && !connectat && !(Tpeons(jo) & (AtacsdePeons[opo][davantpeo]))) { //si no t� als costats o (als costats darrera) cap pe� del propi color, d�bil
				thp->Debils |= Bit(i8);
				if (Tpeons(opo) & Bit(davantpeo)) { //si la casella de devant est� ocupada per un pe�, t� menys penalitzaci�
					i -= PenalitzacioPeoOcupaDavantDebil;
				}
				else {
					i -= PenalitzacioPeoDebil[w];
					//si a m�s est� en una columna semi oberta, m�s penalitzaci�
					if (!peonsdavantoponent)
						;// i -= PenalitzacioExtraPeoDebilColumnaSemiOberta;
					else {
						//si no pot ava�ar immediatament per tenir dos peons rivals atacant al davant, tb considerar-lo bloquejat
				...
				//si l'estan atacant, m�s penalitzaci�
				//d�bil al costat de columna oberta i atac peo contrari al davant, m�s penalitzaci�
	...
	//si la puntuaci� del pe� �s molt bona per� ha deixat un pe� d�bil al costat, menys bonus
...
int avaluacio_peons_2(tss * RESTRICT ss, struct InfoEval *ie) {
	...
	for (a = (ie->thp->Passats & mascarapassatsdebils) & Tpeons(jo); a != 0; a &= a - 1) {
		...
		//si peces suportant avan�, m�s bonificaci�
		if (b == c) {
			z += FerPun(PunI(z) >> 2, PunF(z) >> 2) + FerPun(PunI(z) >> 3, PunF(z) >> 3); //24
		}
		else {
			if (b & Bit(i8 + davantpeo)) { //bonus si controla casella seg�ent peo
				z += FerPun(PunI(z) >> 3, PunF(z) >> 3) + FerPun(PunI(z) >> 4, PunF(z) >> 4); //12
			}
			else {
				//penalitzaci� si no controla casella avan� i el rival s� (amb atac o pe�a)
				if ((Tpeces(opo) | estat_actual->atacs[opo]) & Bit(i8 + davantpeo))
					z -= FerPun(0, PunF(z) >> 4); //4
			}
			//si no controla casella coronaci� i peo avan�at, algo menys de valor
			bool fer = false;
			if (jo == blanques) {
		...
		if ((c & Tpeces(opo))) {
			...
			if (c & Tdames(opo))
				z += penalitzacio_dama_parant_peo_passat_final; //penalitzaci� per la dama, doncs �s menys activa
		}
		else {
			...
			if (!(c & estat_actual->atacs[opo])) { //si no hi ha control de cap pe�a contraria de les caselles d'avan�, m�s penalitzaci�
				z += PenalitzacioExtraPeoPassatNoControlatFinal;
				int numpecesopo = popcount(Tpeces(opo) ^ Tpeons(opo));
				//si el rival t� poques peces, doblar valor peo passat no controlat
				if (numpecesopo < 3) {
		...
		//si hi ha m�nim dos caselles sense controlar, el peo no pot ser massa bo
		...
		//si no atacat i defensat, m�s valor
	...
	for (a = ie->thp->Debils & Tpeons(jo); a != 0; a &= a - 1) {
		...
		//si no estan atacant la casella de davant ni est� ocupada pel rival, menys penalitzaci�
...
int avaluacio_peons(tss * RESTRICT ss, struct InfoEval *ie){
	...
	if (thp->hash != estat_actual->hashpeons) {
		...
		//si el rei est� lluny en vertical d'on hi ha els peons al final
		...
		//peonsdavant. S'aprofita Passats per evitar afegir m�s mem�ria a hash peons
		Bitboard b2 = Tpeons(blanques) & FilesDavant[blanques][r];
		Bitboard pd = 3 - popcount(b2 & ie->bitboard_casellesrei[blanques]) + (!!(Tpeons(blanques) & AvancPeonsPassats[blanques][ie->rei[blanques]] & estat_actual->atacspeons[negres])); //atacs peons rivals compten com un pe� menys
		thp->Passats |= pd;
		thp->SeguretatRei[blanques] = pd * MulPeonsDavant;

		b2 = Tpeons(negres) & FilesDavant[negres][rn];
		pd = ((3 - popcount(b2 & ie->bitboard_casellesrei[negres]) + (!!(Tpeons(negres) & AvancPeonsPassats[negres][ie->rei[negres]] & estat_actual->atacspeons[blanques])))); //atacs peons rivals compten com un pe� menys
		thp->Passats |= pd << 56;
		thp->SeguretatRei[negres] = pd * MulPeonsDavant;

		//si el rei est� lluny en horitzontal d'on hi ha els peons al final
		int estpb = (!!(Tpeons(blanques) & COLUMNAA)) |   //** potser marcar-ho al fer moviment, per anar m�s r�pid
		...
		//per penalitzaci� rei 2a fila i no t� peo al costat llarg
		if (r == 1
		...
		//avaluar estructura peons pr�pia davant rei
		int y = ie->rei[blanques];
		if (ROW(y) > 0) {
			y += Avancar(negres); //tamb� costats
		}
		if ((y == 0 || y == 7) && (Tpeons(negres) & Bit(y + 8)))
			thp->SeguretatRei[blanques] -= 15; //pe� contrari davant del rei el protegeig
		...
		if (!b)
			...
			b ^= Bit(z);
			if (x == 0)
				x = 255; //perqu� no quadri mai
			if (y == 0)
				y = 255; //perqu� no quadri mai
			if (z == 0)
				z = 255; //perqu� no quadri mai
			int w = abs(x - z);
			x = abs(x - y);
			y = abs(y - z);
		...
		//atac de peons
		b = AvancPeonsPassats[blanques][ie->rei[blanques]] & Tpeons(negres) & (FILA2 | FILA3 | FILA4 | FILA5); //peons contraris davant i al costat del rei, avan�ats
		if (varis_bits(b)) {
		...
		//si rei a flanc i pe�a contraria a f3/c3, penalitzaci� extra
		...
		y = ie->rei[negres];
		if (ROW(y) < 7) {
			y += Avancar(blanques); //tamb� costats
		}
		if ((y == 56 || y == 63) && (Tpeons(blanques) & Bit(y - 8)))
			thp->SeguretatRei[negres] -= 15; //pe� contrari davant del rei el protegeig
		...
		//atac de peons
		b = AvancPeonsPassats[negres][ie->rei[negres]] & Tpeons(blanques) & (FILA7 | FILA6 | FILA5 | FILA4); //peons contraris davant i al costat del rei, avan�ats
		if (varis_bits(b)) {
		...
		//si rei a flanc i pe�a contraria a f6/c6, penalitzaci� extra
	...
	//Peons que potser no poden ser parats, en finals de nom�s peons
...
int avaluacioamenaces(tss * RESTRICT ss, struct InfoEval *ie) {
	...
	if (b) {
		...
		if (b) {
			if (c == negres) //treure els peons que el contrari amena�a amb els seus peons, si juga el contrari
	...
	b = (
		(((Tpeons(blanques) & ~FILA7) | ((((Tpeons(blanques) & FILA2)) << 8) & ~TTotespeces)) << 8) //peons que no est�n a 7a, i afegir peons de segona no bloquejats a 3a que poden avan�ar dues caselles
		& ~TTotespeces) & ~estat_actual->atacspeons[negres];
	//que casella d'avan� estigui defensada per nosaltres, o que no estigui atacada pel rival
	...
	if (b) {
		...
		if (b) {
			if (c == blanques) //treure els peons que el contrari amena�a amb els seus peons, si juga el contrari
	...
	//possibles atacs de peons si avancen
	b = (
		(((Tpeons(negres) & ~FILA2) | ((((Tpeons(negres) & FILA7)) >> 8) & ~TTotespeces)) >> 8) //peons que no est�n a 2a, i afegir peons de 7a no bloquejats a 6a que poden avan�ar dues caselles
...
int imbalanc_material(tss * RESTRICT ss, struct InfoEval *ie){
	...
	//compensar una mica difer�ncia material si hi ha molta difer�ncia de mobilitat
	if ((x = PunI(estat_actual->valorpeces[blanques] - estat_actual->valorpeces[negres])) != 0) {
		...
		if (!(x < 0 && y < 0) || (x > 0 && y > 0)) { //no compensar si difer�ncia material i mobilitat son del mateix signe
	...
	if (abs(x) >= vcaballsee)  //no cal per difer�ncies tan grans
		return 0;
	...
	//si molt material, menys compensaci�
	int z3 = 0;
...
int avaluacio(tss * RESTRICT ss) {
	...
	int punt = 0;
	int z;
	bool peonsdosflancs = ((Tpeons(blanques) | Tpeons(negres)) & Mascara_abc) && ((Tpeons(blanques) | Tpeons(negres)) & Mascara_fgh);
  int xx = espai(ss, &ie);

	//Mirar si algun final est�ndard (fins a dama i 2 peons)
	...
	//bonus per canvi de peces si avantatja material  //** revisar si aix� t� sentit
	...
	if (estat_actual->numpeces[AlfilBlanc] + estat_actual->numpeces[TorreBlanca] != estat_actual->numpeces[AlfilNegre] + estat_actual->numpeces[TorreNegre]) {
		z = popcount((Tpeons(blanques) | Tpeons(negres)) & (Columnes[3] | Columnes[4])); //n�m de peons centrals
		if (estat_actual->numpeces[AlfilBlanc] + estat_actual->numpeces[TorreBlanca] > estat_actual->numpeces[AlfilNegre] + estat_actual->numpeces[TorreNegre]) {
	...
	if (ie.thp->flags & HashPeonsFlagCentreBloquejat) {
		//si la posici� est� molt tancada, qui t� m�s espai t� avantatja
		if (xx > 0) {
			punt += 3;
		}
		if (xx < 0) {
		...
		if (estat_actual->numpeces[AlfilBlanc] != estat_actual->numpeces[AlfilNegre]) {  //** quan �s alfil per torre (qualitat de menys) o alfil i caball per torre, potser no t� molt sentit aix�. es pot senzillament comprovar que el n�m de torres siguin iguals
		//alfil �s millor si hi ha peons als dos flancs
	...
	//alfils diferent color amb m�s peces
	...
	if (PunI(punt) != PunF(punt)) {
		...
		int fase;
		if (ie.taulamat == NULL)
			fase = 0; //per defecte suposa que moltes peces, doncs supera m�nim de peces per poder agafar un index existent a taulamat
		else
	...
	//Mirar si algun final est�ndard
	if (ie.vpb <= 1500 && ie.vpn <= 1500) {
		//KRBKR
		if (ie.vpb == valfil + vtorre && ie.vpn_np == vtorre && punt > 100)
			punt = punt >> 3;  //sino creu que t� molta avantatja, quan molts cops son taules
		...
		if (ie.vpb == vtorre + vpeo && (ie.vpn == valfil + vpeo || ie.vpn == valfil + vpeo + vpeo) && punt > 70 && (ie.thp->Passats & mascarapassatsdebils) == 0)
			punt -= punt >> 2;  //sino creu que t� molta avantatja, quan molts cops son taules
		...
		if (ie.vpb_np == valfil && ie.vpn_np == valfil /*&& (punt>100 || punt<-100)*/) {
			...
			if (difcolor) {
				if (ie.thp->Passats) {
					if (VarisBits(ie.thp->Passats & Tpeons(blanques)) || (ie.thp->flags & HashPeonsFlagBlanquesCandidataPassat))
						punt = (punt >> 2) + (punt >> 1);  //sino creu que t� molta avantatja, quan molts cops son taules*/
					else {
			...
			//KBPKB si nom�s hi ha un peo (o 2 i estan doblats), mirar si quasi taules
			z = -1;
			...
			if (z != -1) { //entra aqu� si peons doblats
				if (estat_actual->numpeces[PeoBlanc] >= 1) {
					if (ie.rei[negres] > z && COL(ie.rei[negres]) == COL(z)) //si rei parant peo
						punt = punt >> 3;
					else
		...
		//m�s dif�cil guanyar si no hi ha peons
...
acabar:
	...
	//si prop de la regla de 50 moviments, disminuir avaluaci�
