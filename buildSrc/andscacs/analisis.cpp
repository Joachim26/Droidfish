...
struct tss ssbase[MaxThreads + 1];  //el darrer �s per sincronitzar threads
...
ClauHash obtenir_clau_refutacio3(tss * RESTRICT ss)
{
	...
	if (estat_actual->moviment_previ == JugadaInvalida)
		return 0;
	return p2->hash ^ estat_actual->hash;  //�s correcte, no cal p->hash doncs fent aix� est� traient totes les jugades anteriors
}

void buidar_refutacio3() {
	memset(refutacio3, 0, sizeof(refutacio3));
}
//fi refutaci� tres jugades
...
void mostrar_pv(tss * RESTRICT ss, int score, int alpha, int beta, int pvactual, int mirarfinsnivell) {
	...
	if (temps_per_moure == 0 || temps_per_moure > 1000)
		if ((texte[0] == 0 || (temps_per_moure > 10000 && mirarfinsnivell > 10)) || temps_per_moure == 0) { //lower/upper nom�s si hi ha encara m�s temps
			...
			for (int i = 1; i < NumThreads; i++) {
				numjugadesaltresthreads += ssbase[i].numjugadesfetes;  //aix� suma encara que no hagi acabat el thread. no ve d'aqu�
#ifdef TB
				tbhits += ssbase[i].tbhits;
#endif
			}
			texte2[0] = 0;
			if (score >= maxbeta - MaxProfunditat) {//mat el m�dul d'an�lisi
...
bool mirar_si_parar_root() {
	...
	if (temps_tipus & TipusTempsMoviment)
		return (tempsquehapassat > temps_TempsMoviment);
	return ((tempsquehapassat > temps_maxim) || (int64_t)((double)tempsquehapassat * (double)2.2) > temps_per_moure); //no tindr� temps per acabar el seg�ent ply
}
...
void analitzar(){
	...
	ssbase[0].acabatbe = false; //per marcar si �s la que acava sencera
	...
	if (NumThreads > 1)
		ssbase[MaxThreads] = ssbase[0];  //els altres threads es c�pien aix�
#ifdef RANDOM
	jugadarandom = 0;
	mt_seed();
#endif
	...
	for (ssbase[0].mirarfinsnivellbase = 2; ssbase[0].mirarfinsnivellbase <= mirarfinsprofunditat; ssbase[0].mirarfinsnivellbase++) {
		...
		if (b) {
			if (!temps_per_moure || (temps_tipus & TipusTempsMoviment))  //no �s un control de temps est�ndard
				break;
			if (tempsquehapassat > temps_maxim)
				break;
			...
			if (tempsquehapassat > temps_maxim * 0.6 || canviadajugadabona <= (ssbase[0].mirarfinsnivellbase >> 1)) {
				//a m�s la millor jugada no ha d'haver canviat en la darrera iteraci�
				//ni tampoc l'avaluaci� ha d'estar empitjorant molt
		...
		for (int pvactual = 0; pvactual < multipv && !temps_acabat_real; pvactual++)
		{
			...
			for (;;) {
				...
				if (usarthreads) {
					...
					for (int i = 0; i < NumThreads; i++) {
						ssbase[i].acabatroot = false; //perqu� comenci
					}
				}
				ssbase[0].temps_acabat = false; //ho pot haver posat a true algun altre thread
			...
			if (multipv == 1) { //nom�s es fa quan no multipv
			...
			if (!((tasca == tasca_analitzar && (temps_tipus & TipusInfinit))) && (score == maxbeta || score == maxalpha || (ssbase[0].estat[0].moviments[1].moviment == JugadaInvalida && ssbase[0].mirarfinsnivellbase >= 4)))
				break; //nom�s una jugada possible
		} //pvactual
		if (!((tasca == tasca_analitzar && (temps_tipus & TipusInfinit))) && (score == maxbeta || score == maxalpha || (ssbase[0].estat[0].moviments[1].moviment == JugadaInvalida && ssbase[0].mirarfinsnivellbase >= 4))) {
			break;  //nom�s una jugada possible
		}
...
fipertb:
	//esperar a rebre comanda per sortir si est� en modo ponder o infinit
...
void proces_thread(void *param) {
	...
	void* proces_thread(void *param) {
...
#ifndef ES32BITS
		if (WinGroupAffinity)
			WinProcGroup::bindThisThread((size_t)numthread);
#endif
#endif
#ifdef LINUX
		long int numthread = (long int)param;
		...
		do {
			...
			if (!paramroot.analitzant) {
				...
				//no ocupar cpu mentre no est� pensant
#ifdef WINDOWS
				Sleep(1);
#endif
#ifdef LINUX
	...
	void search_root(long int numthread, int pvactual) {
		...
		if (numthread != 0)
			ssbase[numthread] = ssbase[MaxThreads];  //aix� actualitza entre altres mirarfinsnivellbase
		...
		if (numthread != 0 && paramroot.nointentarcomencar)
			return;
		//mirar si est� en escac
		if (mirarfinsnivell == 1) {//generar jugades nom�s 1r cop, sense il�legals
			const int jo = ss->estat_actual->mou;
			ss->mov = (ss->estat_actual + 1)->moviments;  //posar els moviments a estat_actual+1 per anar agafant nom�s els legals
			...
			while (ml->moviment) {
				...
				//puntuaci� inicial segons avaluaci�
				...
				int punt = -avaluacio(ss); //negatiu pq �s valoraci� de cara a qui mou despr�s de la jugada
		...
		if (ss->numthread == 0 && mirarfinsnivell == 1) {
			//el primer cop ja es decideix quina ser� la jugada bona, tot i que fa l'an�lisi complert normalment
		...
		int antpuntpv[100];
		//donar puntuaci� m�xima al millor moviment de la iteraci� anterior
		if (mirarfinsnivell > 1 || ssbase[0].MillorsMovimentsPv[pvactual] != JugadaInvalida) {
			...
			//NO: ordenar resta de jugades segons n�mero de jugades que ha mirat en depth anterior
			// no cambia ordre 1r element, doncs �s el millor
			OrdenacioEstableJugades(ss->estat_actual);
		}
		...
		while (movact->moviment) {
			...
			if (movact->puntuacio > auxll) {
				movact->puntuacio = antpuntpv[abs((long long)(movact->puntuacio - maxpuntuacio - 100))]; //recuperar puntuaci� que hi havia
				movact++;
			...
			if (ss->nummovimentlegal == 1) //si �s el 1r moviment
		...
		temps_acabat_root:
			...
			if (score > alpha) {
				...
				if (ss->seldepth < mirarfinsnivell)
					ss->seldepth = mirarfinsnivell; //aix� estalvia incrementar-ho dins alpha_beta;
				...
				ss->seldepth = 0;  //�s la profunditat m�xima que ha usat per analitzar la variant principal
				if (score >= beta) {
					ss->score = score;
					ss->acabatroot = true;
					ss->acabatbe = true;
	...
	int alpha_beta(tss * RESTRICT ss, int alpha, int beta, int n, bool pot_null, int mirarfinsnivell, bool cutNode, bool ProvarMoviments) {
		...
		//extensi� per canvi de pe�a a l�nies_pv
	...
	no_fer_hashab:
			...
			if (estat_actual->fase == etapa_bones_captures ||
				...
				jugadapotserdolenta:
					if (estat_actual->seguentsegonesjugades - &estat_actual->segonesjugades[0] >= MAX_SEGONESJUGADES - 1)
						goto fer_jugada_hash; //no n'hi caben m�s
		...
		fer_jugada_hash:
			...
			//redu�r una mica captures o killers
			if (!fenthash
			...
			//extensi� per peo arribar a 7a
			if (!extes && !extensioescac &&
				((ROW(Desti(m)) == 6 && ss->tau.c[Desti(m)] == PeoBlanc) || (ROW(Desti(m)) == 1 && ss->tau.c[Desti(m)] == PeoNegre)) &&
				(estat_actual->actual == NULL || estat_actual->actual->puntuacio > valorcapturesdolentes)) { //no fer-ho si t� SEE negatiu
			...
			if (reduc_lmr > 0 && !FetaExtensioPeo7a)		//** no t� molt sentit que pugui haver ext�s i fer lmr igualment
			{
				//redu�r menys si pe�a escapa d'atac
			...
			if (feranalisinormal) {
				...
				if (!(es_pv && nummovimentlegal == 1)) { //no es fa si �s pv i �s la 1a jugada
	...
	int quiesce(tss * RESTRICT ss, int alpha, int beta, int n, int mirarescacs, int nivell) {
		...
		if (!estat_actual->escac) {
			...
			if ((alpha != beta - 1) && alpha < standpat) //nom�s si pv_node
				alpha = standpat;

		}
		else {
			standpat = maxalpha;
			estat_actual->clavatsrival = 0; //que afecti a quiesce, nom�s s'usa per avaluaci�
		}
		if (!calculat) {
			if (estat_actual->calculaatacs) {
				calcular_atacs(ss);
	...
	no_fer_hash:
		...
		generar_cc(ss);
		estat_actual->fi = ss->mov;  //per continuar generant moviments a partir d'aqu� si s'escau
		marcar_captures_bones_q(ss);
		estat_actual->actual = &estat_actual->moviments[0];
	...
	int son_taules(tss * RESTRICT ss, int n) {
		...
		//taules per 50 moviments i per repetici�
	...
	int see(tss * RESTRICT ss, int moviment, int marge, e_colors jo) {
		...
		if (ss->estat_actual->descoberts & Bit(from))
			return 1; //�s escac descobert. Analitzar
		if ((ss->estat_actual->clavats & Bit(from)) && valorpecamou <= valfilsee)
			return 1;  //mou una pe�a clavada. Analitzar. //** mirar si �s bo return 1
		...
		do {
			...
			//trobar i treure la pe�a que captura de menys valor
			...
			{
				llista[++index] = vdamasee * 16; //assegurar que no agafa aquest valor mai, que si per exemple es posa vdamasee*2 podria ser m�s petit que el valor real
				break;
			}
