                                     ...
                                     MOVE threat_move)
{
    ...
    switch (search_mode)
    {
        ...
        case TR_ADJ: /*4*/
        {
            ...
            i = 0;
            do {
                if (((movelist[i].u ^ threat_move.u) & mv_mask) == 0)
                {
                ...
                if (((movelist[i].u ^ pv_move.u) & mv_mask) == 0)
                {
                    ...
                    *should_iid = 0;
                    if (search_done == TR_ADJ) /*both found*/
                        return;
                    search_done = PV_ADJ;
                } else if (((movelist[i].u ^ threat_move.u) & mv_mask) == 0)
                {
                    movelist[i].m.mvv_lva = MVV_LVA_THREAT; /* Move found as threat from previous level Null Move search */
                    if (search_done == PV_ADJ) /*both found*/
                        return;
    ...
    } else
    {
        ...
        if ((!is_pv_node) && (!being_in_check))
        {
            ...
            if (CanNull && (depth >= NULL_START_DEPTH) && (is_material_enough >= NULL_PIECES))
            {
                ...
                t = -Search_Negascout(0, level + 1, &line, x2movelst, 0, null_depth, -beta, -beta + 1, next_colour, &iret, 0, 0, smove, 0);
                if (t >= beta)
                    return t;
                if (iret >= 0)
                     ...
                     uint64_t *restrict spent_nodes, int64_t *restrict spent_time)
{
    ...
    *spent_time = 0LL;
    ...
    if (given_moves_len > 0)
    {
        ...
        {
            if (move_cnt == actual_move_cnt)
                given_moves_len = 0; /*full move list selected*/
            else
                move_cnt = actual_move_cnt;
        }
    ...
    if ((disable_book == 0) && (full_move_time < INFINITE_TIME) &&
        (Book_Is_Line(&ret_mv_idx, movelist, move_cnt, given_moves_len)))
    {
                ...
                } else if (nscore >= beta) /*fail high*/
                {
                    ...
                    if (ret_mv_idx > 0) /*not already the main PV?*/
                    {
                        ...
                        /*found new best move. update ranking and PV immediately to get this
                          ...
                          move didn't.*/
                        Search_Find_Put_To_Top_Root(movelist, opp_move_cache, move_cnt, ret_move);
                        /*update global PV*/
                        ...
                        } else if (opp_move_cache[0] != MV_NO_MOVE_CMASK)
                        {
                            ...
                            GlobalPV.line_cmoves[1] = opp_move_cache[0];
                            GlobalPV.line_len = 2;
                        } else
