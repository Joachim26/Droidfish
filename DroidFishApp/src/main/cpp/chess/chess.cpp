...
/*
    Kōhai 1.0 Copyright (C) 2016 Michael Byrne.
    (A direct derivative of Senpai 1.0 by Fabien Letouzey (C) 2014,.
...
const int PAWN_VALUE   =  90;
const int KNIGHT_VALUE = 300;
const int BISHOP_VALUE = 300;
const int ROOK_VALUE   = 490;
const int QUEEN_VALUE  = 1010;
...
int reduction(Search_Local & /* sl */, int /* mv */, int depth, bool /* pv_node */, bool /* in_check */, int searched_size, bool dangerous) {
   ...
   if (depth >= 3 && searched_size >= 3 && !dangerous) {
      red = (searched_size >= 6) ? depth / 2.7 : 1;
   }
...
void command(Scanner & scan) {
   ...
   if (false) {
      ...
      std::cout << "option name Cores type spin default " << engine::engine.threads << " min 1 max 8" << std::endl;
